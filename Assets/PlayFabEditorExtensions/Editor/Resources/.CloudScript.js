...
handlers.UpdateHighScore = function (args, context)
{
    ...
    return "UHS Ran"; ///Check Return(Debug.Log(result.return));
};
...
handlers.UpdateInputScore = function (args, context)
{
    var score = args.score;
    ...
    var request =
        {
            PlayFabId: currentPlayerId,
            Statistics: [{ StatisticName: "FewestInputs", Value: score }]
        };

    var result = server.UpdatePlayerStatistics(request);

    return "UIS Ran"; ///Check Return(Debug.Log(result.return));
};
...
handlers.AcceptTrade = function (args, context)
{
    var tradeOwner = server.GetUserAccountInfo({PlayFabId: args.tradeOwnerId}).UserInfo.TitleInfo.TitlePlayerAccount;

    var removeMembersRequest =
        {
            Group:
                {
                    Id: "[A30D93ECD50A016B]",
    ...
    var removeMembersResult = entity.RemoveMembers(removeMembersRequest);
...
handlers.AddNewTradeOffer = function (args, context)
{
    ...
    var addMembersRequest =
        {
            ...
            Members: [titlePlayerAccount]
        };

    var addMembersResult = entity.AddMembers(addMembersRequest);

    var playerDataRequest =
        {
            PlayFabId: currentPlayerId,
            Data: { "CurTradeID": tradeID}
        };
    var playerDataResult = server.UpdateUserInternalData(playerDataRequest);
};


//Region Trade Display

handlers.GetTradeIDs = function (args, context)
{
    ...
    var listMembersResult = entity.ListGroupMembers(listMembersRequest);




    var members = listMembersResult.Members[1].Members;


    var playerIdList = [];
    var playerDisplayNameList = [];
    var tradeIdList = [];


    var x;
    for(x = 0; x < members.length; x++) {
        var playerID = members[x].Lineage["1FD0CD212316F6FE"].Id;

        if (playerID == currentPlayerId)
            continue;

        var playerData = server.GetPlayerProfile({PlayFabId: playerID});

        var tradeDataRequest =
            {
                PlayFabId: playerID,
                Keys: ["CurTradeID"]
            };

        var tradeDataResult = server.GetUserInternalData(tradeDataRequest);
        var TradeID = tradeDataResult.Data["CurTradeID"].Value;

        playerIdList.push(playerID);
        playerDisplayNameList.push(playerData.PlayerProfile.DisplayName);
        tradeIdList.push(tradeID);
    }
    ...
    var returnData =
        {
            ...
            playerDisplayNames: playerDisplayNameList,
